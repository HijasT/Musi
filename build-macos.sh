...
iconutil -c icns icon.iconset -o ChaosMD.icns
...
if [ -d "dist/ChaosMD.app" ]; then
    cp -r dist/ChaosMD.app dmg_contents/
else
    cp -r dist/ChaosMD dmg_contents/
fi

# Add Applications symlink so users can drag to install
ln -s /Applications dmg_contents/Applications

DMG_NAME="ChaosMD-macos-${ARCH}.dmg"
hdiutil create -volname "ChaosMD" -srcfolder dmg_contents -ov -format UDZO "$DMG_NAME"

# Cleanup build artifacts
rm -rf build icon.iconset dmg_contents ChaosMD.icns bin

echo ""
echo "=== Build complete ==="
echo "Output: $DMG_NAME"
echo ""
echo "To install: open the DMG and drag ChaosMD to Applications."
